...
当前版本已经具备可用的基础单音扒谱工作流，并增加了实验性和弦/复音与逻辑多轨模式：导入和播放音频、可选分离吉他 stem、检测音高和和弦、将单音/和弦事件分入主音/节奏候选轨、生成并编辑六线谱，以及保存项目或导出 TXT、MIDI 和 MusicXML。真正的吉他内部音频分轨和演奏技巧识别仍在后续阶段。
...
Phase 9B（逻辑主音/节奏轨分类与多轨管理）已完成基础版。完整歌曲可先使用 Demucs 提取综合吉他 stem，再选择单音或实验性复音分析。程序会将清理后的单音段分入主音候选轨，将和弦/同时音分入节奏候选轨，将低可信度结果保留在未分类轨。每条轨可独立编辑、撤销/重做、修改角色、保存和导出。
...
1. 用同时包含单音 riff 和和弦的授权素材验证 Phase 9B 轨道归类准确率。
2. Phase 9C：滑弦、击勾弦、推弦和颤音识别。
