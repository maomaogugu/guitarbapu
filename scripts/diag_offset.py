"""Scan offset score landscape for the current analysis."""
...
def main() -> None:
    ...
    print(f"notes={len(notes)} targets={len(targets)} span={span:.2f} duration={audio.duration:.2f}")

    results = []
    for offset in np.arange(0.0, audio.duration - span, 0.5):
        hits = 0
        midi_hits = 0
        for time, string, fret, _m, _t in targets:
            ...
            if mask.any():
                if np.isin(midis[mask], (tgt,)).any():
                    hits += 1
                    midi_hits += 1
                elif np.isin(midis[mask], (tgt - 12, tgt + 12)).any():
                    midi_hits += 1
        results.append((round(offset, 1), hits, midi_hits))
    results.sort(key=lambda x: x[1], reverse=True)
    for r in results[:20]:
        print(f"offset={r[0]:6.1f} strict_hits={r[1]:3d} midi_hits={r[2]:3d} ({r[1]/len(targets):.3f})")
